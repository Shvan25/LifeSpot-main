...
namespace LifeSpot
{
    public static class EndpointMapper
    {
        ...
        /// <summary>
        ///  Маппинг slider-файлов
        /// </summary>
        public static void MapSlider(this IEndpointRouteBuilder builder)
        {
            var sliderFiles = new[] { "london.jpg", "new_york.jpg", "SP.jpg" };

            foreach (var fileName in sliderFiles)
            {
                builder.MapGet($"/slider/{fileName}", async context =>
                {
                    var sliderPath = Path.Combine(Directory.GetCurrentDirectory(), "slider", fileName);
                    // var slider = await File.ReadAllTextAsync(sliderPath);
                    // await context.Response.WriteAsync(slider);
                    await context.Response.SendFileAsync(sliderPath);
                });
            }
        }
    }
